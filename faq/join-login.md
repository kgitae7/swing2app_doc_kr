...
스윙투앱 홈페이지에서 간편로그인(SNS 로그인)으로 가입하신 경우 비밀번호 찾기나 변경은 **불가능**합니다.

간편로그인은 구글, 네이버, 카카오 등 외부 SNS 계정의 인증 정보를 사용하기 때문에 스윙투앱 자체 비밀번호가 설정되지 않습니다. 따라서 비밀번호 찾기나 변경 기능을 이용하실 수 없습니다.

**로그인이 안 되는 경우 확인사항:**

* 가입 시 사용한 SNS 계정(구글/네이버/카카오)이 맞는지 확인해 주세요
* 비밀번호는 연결한 계정의 비밀번호 확인(구글 계정이라면 구글 지메일로 들어가서 사용중인 계정 확인해주세요)
* 해당 SNS 계정 자체의 로그인 상태를 확인해 주세요
* 여러 번 실패했다면 브라우저 캐시를 삭제하거나 다른 브라우저에서 시도해 보세요

만약 일반 이메일 계정으로 가입하셨다면 로그인 화면에서 '비밀번호 찾기'를 통해 재설정하실 수 있습니다.
...
* 부관리자 계정은 앱 관리자가 먼저 설정해야 사용할 수 있습니다.
* 관리자가 부관리자를 등록하지 않았다면 로그인이 불가능합니다. -앱 제작이 완료되어야 부관리자 로그인이 가능합니다.
